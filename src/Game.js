...
export default function Game(props) {
    ...
    const children = ({ remainingTime }) => {
        ...
        if(remainingTime === 0) {
            return <div className="timer-end" role="timer" aria-live="assertive">
            Time's up!
            </div>
        }
        // Return the remaining seconds 
        return <div className="countdown" role="timer" aria-live="assertive">
            {remainingTime}
        </div>
    }
    ...
    function onEndOfGame() {
        ...
        db.collection("quizine-leaderboard").add({
        ...
        <main className="background">
            <div className="game-heading">
                <div className="timer-container">
                    ...
                    <div className="glow">
                        <CountdownCircleTimer className="glow"
                            isPlaying={isPlaying}
                ...
                <button className="exit" onClick={endGame}>Exit Game</button>
            </div>
            <div className="game-display">
                <div className="score">
                    <p>Your Score: {userScore} of {totalAnswered}</p>
                </div>
                
                {props.questionData !== undefined && props.questionData.map((question, index) => {
                    if (questionIndex === index) {
                        return <Question key={"question" + index} isValidTime={isValidTime} questionData={question}
                ...
                {showTimesUp && 
                    <div className="popup-background">
                        ...
                        <i className="material-symbols-rounded" style={{fontSize:'2.5rem', fontWeight: '700'}}>alarm_off</i><span>Timeout!</span>
                        </div>
                    </div>
                }
                {roundOver && <GameSummary onExit={endGame} gameDetails={props.gameDetails} leaderboardData={leaderboardData} currentDocId={docIdRef}/> }
