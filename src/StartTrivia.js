...
export default function StartTrivia(props) {
    const [visible, setVisibility] = React.useState(props.entryInfo.show_settings);
    const [settings, setSettings] = React.useState({
        category: props.entryInfo.selected_category ? props.entryInfo.selected_category : "general_knowledge",
        question_count: 10,
        timer: 10,
    });
    ...
    function onStartGame(e) {
        ...
        if(avatarIndex!== undefined && playerName !== undefined){
            ...
            fetch(`https://the-trivia-api.com/api/questions?limit=${gameDetails.game_settings.question_count}&categories=${gameDetails.game_settings.category}`)
        ...
        <main style={{marginBottom:'0'}}>
            <div className="hero-banner banner-content-centered">
                <div className="banner-content-centered">
                    <h1>Time to play!</h1>
                    <p>Before we start, check the game settings to personalize your Trivia.</p>
                    <div id="main">
                        <button id="settings" onClick={handleSettingsPopup}>
                            <i className="material-symbols-rounded" style={{fontVariationSettings: "'FILL' 1"}}>settings</i>
                            Settings
                        </button>
                    </div>
                    ...
                    {visible && <Settings
                    settings={settings} onSettingsCloseCallback={handleSettingsPopup}
                ...
                <form id="player-customization">
                    <PlayerCustomization updatePlayerName={updatePlayerName}  updateAvatarIndex={updateAvatarIndex} showAvatarError={showAvatarError}/>
                    <button className="start-game" onClick={onStartGame} disabled={disable}>Start game</button>
                </form>
            </div>
            {isLoading && <Loading />}
