...
export default function PlayerCustomization(props) {
    ...
    return <React.Fragment>
        <fieldset >
            ...
            <input type="text" placeholder="Enter your name" id="player_name" name="player_name" ref={inputRef} onChange={onPlayerNameInput} required />
            ...
            <div className="avatar-grid" id="avatar-grid">
                ...
                {listOfImgs.map((item, index) => {
                    const selectClassName = selectedImageIndex === index ? "selected-avatar" : "";
                    return <div className={`avatar-circle ${selectClassName}`} key={'avatar'+ index} onClick={ 
            ...
            {props.showAvatarError && <p className="error">Please select an avatar!</p>}
        </fieldset>
